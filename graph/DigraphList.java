...
public class DigraphList extends AbstractGraph {
        ...
        for (var i = 0; i < getNumberOfVertices(); i++) {
            for (var j = 0; j < getAdjacencyList().get(i).size(); ++j) {
                ...
                g.add(mutNode(getVertices().get(i).getName())
                        .addLink(to((mutNode(getVertices().get(destinationIndex).getName())))
                                .add(Label.of(String.valueOf(weight)))));
            }
        }
        try {
            Graphviz.fromGraph(g).width(GRAPHVIZ_IMAGE_WIDTH).render(Format.PNG)
                    .toFile(new File(GRAPHVIZ_FOLDER + fileName + GRAPHVIZ_FILE_EXTENSION));
        } catch (IOException e) {
    ...
    public Vertex getNextConnectedVertex(Vertex source, Vertex currentConnection) {
        ...
        while (getAdjacencyList().get(vertexIndex).get(currentAdjacentVertexIndex)
                .getDestination() != currentConnection) {
            currentAdjacentVertexIndex++;
        }
        currentAdjacentVertexIndex++;
        if (getAdjacencyList().get(vertexIndex).size() > currentAdjacentVertexIndex) {
    ...
    protected DigraphList clone() throws CloneNotSupportedException {
        ...
        for (int i = 0; i < getAdjacencyList().size(); i++) {
            for (int j = 0; j < getAdjacencyList().get(i).size(); j++) {
                Vertex destinationInCloneGraph = cloneGraph.getVertices()
                        .get(getVertices().indexOf(getAdjacencyList().get(i).get(j).getDestination()));
                cloneGraph.addEdge(cloneGraph.getVertices().get(i), destinationInCloneGraph,
                        getAdjacencyList().get(i).get(j).getWeight());
            }
        }
        return cloneGraph;
    }
